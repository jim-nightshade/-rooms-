...
def main(game, player):
    ...
    with open(player.name + '\'s path.txt', 'a') as file:
        file.write(player_path + '\n' + player_eq_val + '\n')

    with open(player.name + '\'s path.txt', 'r') as file:
        game_score = [line.rstrip('\n') for line in file]

    top_score = sorted([int(score) for score in game_score[1:len(game_score):2]])
    print('Your Highest SCORE is: ', top_score[-1])
